    ...
    _buffer(nullptr)
{
    ...
    while (*h)
    {
        ...
        if (QString(name).startsWith("default"))
        {
            ...
            qDebug() << desc;
            QString temp(desc);
            addDevice(temp.left(temp.indexOf(',')).toLocal8Bit().constData());
            free(desc);
        }
                         ...
                         const unsigned int channels, const unsigned int prec)
{
    ...
    const int err = snd_pcm_open(&_pcmHandle, devName.toLocal8Bit().constData(), SND_PCM_STREAM_PLAYBACK, 0);
    if (err < 0)
    {
        qWarning() << "Error: " << snd_strerror(err);
        return 0;
    ...
    initMixer(devName.replace("default", "hw").toLocal8Bit().constData());
